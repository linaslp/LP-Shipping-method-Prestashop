...
class AdminLPShippingOrderSavedController extends ModuleAdminController
{
    const CLASS_NAME = 'AdminLPShippingOrderSavedController';
    ...
    public function renderList() {
        $this->_where = 'AND (status = "' . LPShippingOrder::ORDER_STATUS_SAVED . '"'
        ...
        $this->bulk_actions = [
            ...
            'removeShipmentsBulk' => array('text' => $this->module->l('Remove orders', self::CLASS_NAME), 'icon' => 'icon-cogs'),
        ];
    ...
    public function postProcess()
    {
        ...
        if (Tools::isSubmit('submitBulkformShipmentsBulklpshipping_order')) {
            if (is_array($data)) {
                if (key_exists('lpshipping_orderBox', $data)) {
                    $this->formShipmentsBulk($data['lpshipping_orderBox']);
                }
            } 
        }
    }
    ...
    public function formShipmentsBulk(array $rowIds)
    {
        ...
        if (is_array($errors)) {
            if (!empty($errors)) {
                foreach ($errors as $orderId) {
                    $this->addMessage(false, 'Order ' . $orderId . ' is not saved in LP API. Save it first and then try again');
                }
            } 
